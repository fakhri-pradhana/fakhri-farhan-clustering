...
* Menggunakan library `caret` yang merupakan package untuk *Classification and Regression Training*
* Melakukan data splitting dengan komposisi 70% training dan 30% testing dengan method `createDataPartition()`
* Parameter method `createDataPartition()` antara lain fitur *Ball.Possesion*, jumlah partisi, dan list.
* Mengassign hasil split ke dalam variable `trainingBallPossesion` dan `testingBallPossesion`
* Mengecek jumlah dimensi dari data dengan method `dim()`
...
svm_linearCoba <- train(Ball.Possession.. ~., data = trainingBallPossesion, method = "svmLinear", 
...
```{r}
test_predCoba <- predict(svm_linearCoba, newdata =testingBallPossesion)
test_predCoba
```
